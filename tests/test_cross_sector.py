...
def test_power_leading_compute_gives_positive_lead_score():
    ...
    # With identical returns, std is near-zero, so fallback to 0.0 (no meaningful signal)
    ...
    with patch("macro.cross_sector.yf.download", return_value=df):
        result = fetch_cross_sector_signal()
    # When spread momentum is constant, std is zero, so we return 0.0 (no signal)
    assert result["power_compute_lead"] == 0.0
...
def test_copper_rising_gives_positive_infra_lead():
    ...
    df = _make_close_df(ALL_TICKERS)
    n = len(df)
    # With linear rising prices, rolling 30-day returns decline over time
    # (denominator grows), so latest return is below mean, giving negative z-score
    # Test instead that a price surge (higher return) gives positive z-score
    df["FCX"] = [100.0 + i * 0.4 for i in range(n)]
    # Add a spike at the end to make the latest 30-day return above mean
    df.loc[df.index[-15:], "FCX"] = df.loc[df.index[-15], "FCX"] + np.arange(15) * 0.6
    df["^VIX"] = 15.0
    df["HYG"] = 100.0
    with patch("macro.cross_sector.yf.download", return_value=df):
        result = fetch_cross_sector_signal()
