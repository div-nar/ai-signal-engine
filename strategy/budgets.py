...
def apply_layer_tilt(baseline: dict[str, float], tilt: dict[str, float],
    ...
    With `clamp=False` (full-autonomy mode) the floor/ceiling guardrails are
    off: budgets are floored at 0 (a negative budget is arithmetic nonsense,
    not a judgment call) and renormalized to 1.0 — the LLM may go all-in on a
    single layer if it wants to.
    """
    ...
    if not clamp:
        budgets = {k: max(v, 0.0) for k, v in budgets.items()}
